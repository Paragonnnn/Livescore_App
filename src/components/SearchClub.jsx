...
const SearchClub = ({setSearchClub, windowWidth, showSearch, handleSearchChange, searchClub, clubs, setShowSearch}) => {
  ...
  return (
    <div className=' '>
        ...
        <input onFocus={() => setFocus(true)} onBlur={() => {setFocus(false)}} className={` bg-transparent outline-none hidden lg:block border border-solid border-black p-2 `} onChange={handleSearchChange} value={searchClub} id="" />

        <div className='absolute'>
          <div className={`${toggleSearch ? 'block' : 'hidden'}`}>
            <input onFocus={() => setFocus(true)} onBlur={() => setFocus(false)} className={` bg-transparent outline-none block lg:hidden border border-solid border-black p-2 `} onChange={handleSearchChange} value={searchClub} id="" />

          </div>
          <div className={`${focus ? 'block' : 'hidden'} ${!toggleSearch && windowWidth < 1024 ? 'hidden' : ''}  overflow-y-scroll scroll_bar h-[100vh] lg:h-[80vh] absolute bg-customBg2  border border-solid border-red-600  left-[0] mt-4 lg:w-fit lg:min-w-[250px] p-2 divide-y divide-black`}>
            {
              clubs && 
              clubs.filter(club => (
                ...
                <div key={club.country_key} className='flex gap-4 items-center p-2'>
                  <img src={club.country_logo} alt="" className=' w-8 h-8 rounded-full'/>
                  <Link className=' text-xl text-white' to={`/leagues/${club.country_name}/${club.country_key}`}>{club.country_name}</Link>
                </div>
              ))
            }
            {
              ...
              clubs.filter(club => (
                ...
                <div key={club.league_key} className=' p-2' onClick={console.log('clicked')}>
