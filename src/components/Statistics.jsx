                ...
                stats.map(stat => (
                  ...
                  <div key={s.type}  className={`${toggleMode? 'text-darkText' : 'text-lightText'} divide-y divide-black`}>
                      ...
                      <div className='flex items-center justify-center gap-2 bg-customBg2 w-full py-[2px] rounded px-2'>
                        ...
                        <div className={s.type === 'Ball Possession' ? `w-[${s.home}] ${s.type === 'Ball Possession' && +s.home.slice(0,s.home.indexOf('%')) > +s.away.slice(0,s.away.indexOf('%')) ? 'bg-green-600' : 'bg-red-600' }  h-[6px]` :`w-[${Math.round(((+s.home * 100)/(+s.home + +s.away)))}%] ${+s.home === +s.away ? 'bg-gray-400' : (+s.home > +s.away) ? ' bg-green-500' : 'bg-red-500'}  h-[6px] `}></div>
                        ...
                        <div className={s.type === 'Ball Possession' ? `w-[${s.away}] ${s.type === 'Ball Possession' && +s.home.slice(0,s.home.indexOf('%')) > +s.away.slice(0,s.away.indexOf('%')) ? 'bg-red-600' : 'bg-green-600' }   h-[6px]` :`w-[${Math.round(((+s.away * 100)/(+s.home + +s.away)))}%]  ${+s.home === +s.away ? 'bg-gray-400' : (+s.home > +s.away) ? ' bg-red-500' : 'bg-green-600' }  h-[6px] `}></div>
                        </div>
                      </div>
