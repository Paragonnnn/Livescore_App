...
const Star = ({ team, teamData }) => {
  ...
  const addFavFunc = async (docSnap, docRef, team) => {
    ...
    console.log(
      !Object.keys(docSnap.data().favouritesTeams.teams).includes(
        teamData["team_key"].toString()
      )
    );
    !Object.keys(docSnap.data().favouritesTeams.teams).includes(
      teamData["team_key"].toString()
    ...
    Object.keys(docSnap.data().favouritesTeams.teams).includes(
      ...
      (await updateDoc(docRef, {
        [`favouritesTeams.teams.${team.team_key}`]: deleteField(),
      })
        .then(() => {
          console.log("deleted");
  ...
  const getDocFunc = async (team) => {
    const docRef = doc(db, "users", `${user.uid}`);
    const docSnap = await getDoc(docRef);
    console.log(docRef);
    setFavteams(Object.keys(docSnap.data().favouritesTeams.teams));
    console.log(favTeams);
    console.log(teamData["team_key"].toString());
  };
  useEffect(() => {
    getDocFunc();
  }, []);
  ...
  //     ? await updateDoc(docRef, {
  //         favouritesTeams: {
  //           teams: [...docSnap.data().favouritesTeams.teams, team],
  //         },
  //       })
  //         .then((data) => {
  ...
  return (
    <div
      className={`${
        user ? "block" : "hidden"
      } hover:block h-fit w-fit relative`}
      // onMouseOver={(e) =>
